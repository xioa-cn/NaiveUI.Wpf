...
public sealed class ButtonDocsPageViewModel: ViewModelBase
{
    public ButtonDocsPageViewModel(string selectedKey = "button")
    {
        ...
        ButtonPropsRows =
        [
            new ApiDocRow { Name = "Grade", Type = "Grade", DefaultValue = "Default", Description = "设置按钮语义色阶。\nDefault：默认中性色按钮；\nPrimary：主按钮，用于页面主操作；\nInfo：信息态按钮；\nSuccess：成功态按钮；\nWarning：警告态按钮；\nError：危险或错误态按钮；\nCustomize：启用自定义配色，\n通常配合 CustomBackgroundBrush、CustomBorderBrush、CustomForegroundBrush 等属性使用。" },
            new ApiDocRow { Name = "Kind", Type = "NButtonKind", DefaultValue = "Filled", Description = "设置按钮视觉形态。\nFilled：实心按钮；\nSecondary：次要实色按钮；\nTertiary：浅边框按钮；\nQuaternary：轻量级按钮；\nGhost：透明背景描边按钮；\nDashed：虚线边框按钮；\nText：纯文本按钮。" },
            new ApiDocRow { Name = "Size", Type = "NControlSize", DefaultValue = "Medium", Description = "设置按钮尺寸。\nTiny：超小尺寸；\nSmall：小尺寸；\nMedium：中尺寸；\nLarge：大尺寸。" },
